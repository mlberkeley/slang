...
class BSTM(model.Model):
    ...
    def construct(self):
        ...
        with tf.variable_scope('loss'):
            ...
            with tf.variable_scope('optimizer'):
                ...
                self.optimizer_st = tf.train.AdamOptimizer(self.params['learning_rate'])
                self.optimizer_rc = tf.train.AdamOptimizer(self.params['learning_rate'])
                reconstr_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                                  scope='decoder/curr')
                skipthought_vars = [v for v in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)
                                    if v not in reconstr_vars]
                self.train_step_st = self.optimizer_st.minimize(self.skipthought_l,
                                                                var_list=skipthought_vars)
                self.train_step_rc = self.optimizer_rc.minimize(self.reconstr_l,
                                                                var_list=reconstr_vars)
                tf.summary.scalar('loss', self.skipthought_l + self.reconstr_l)
    ...
    def train(self, batch, step, write_summaries=False):
        ...
        if write_summaries:
            _, _, summary = self.sess.run([self.train_step_st, self.train_step_rc, self.merged],
                                          feed_dict=feed)
            self.writer.add_summary(summary, step)
        else:
            self.sess.run([self.train_step_st, self.train_step_rc], feed_dict=feed)
